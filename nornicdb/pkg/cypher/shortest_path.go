...
func (e *StorageExecutor) parseShortestPathQuery(cypher string) (*ShortestPathQuery, error) {
	...
	funcRe := regexp.MustCompile(`(?i)(allShortestPaths|shortestPath)\s*\(\s*(\([^)]+\)\s*-\[.*?\]->\s*\([^)]+\)|\([^)]+\)\s*<-\[.*?\]-\s*\([^)]+\)|\([^)]+\)\s*-\[.*?\]-\s*\([^)]+\))\s*\)`)
	matches := funcRe.FindStringSubmatch(cypher)
	...
	patternMatches := patternRe.FindStringSubmatch(pattern)
	...
	varRe := regexp.MustCompile(`(?i)(\w+)\s*=\s*(?:all)?shortestPath`)
	if varMatches := varRe.FindStringSubmatch(cypher); varMatches != nil {
...
func (e *StorageExecutor) resolveShortestPathVariables(query *ShortestPathQuery, startVar, endVar string) {
	...
	matchRe := regexp.MustCompile(`(?is)MATCH\s+(.+?)\s+MATCH`)
	matches := matchRe.FindStringSubmatch(query.originalCypher)
