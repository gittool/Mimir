...
func TestFeatureFlags_Enable(t *testing.T) {
	...
	if !IsKalmanEnabled() {
		t.Error("Kalman should be enabled after EnableKalmanFiltering()")
	}
	
	DisableKalmanFiltering()
	if IsKalmanEnabled() {
		t.Error("Kalman should be disabled after DisableKalmanFiltering()")
	}
}
...
func TestFeatureFlags_EnableAllDisableAll(t *testing.T) {
	...
	if !IsFeatureEnabled(FeatureKalmanTemporal) {
...
func TestFeatureFlags_GetFeatureStatus(t *testing.T) {
	...
	if status.Features[FeatureKalmanCoAccess] {
...
func TestPredictIfEnabled(t *testing.T) {
	...
	result := k.PredictIfEnabled(FeatureKalmanDecay, 5)
...
func TestFilteredValue_Fields(t *testing.T) {
	...
	EnableKalmanFiltering()
	result := k.ProcessIfEnabled(FeatureKalmanLatency, 42.0, 50.0)
	
	if result.Raw != 42.0 {
		t.Errorf("Raw = %v, want 42.0", result.Raw)
	}
	if result.Feature != FeatureKalmanLatency {
		t.Errorf("Feature = %v, want %v", result.Feature, FeatureKalmanLatency)
	}
	if !result.WasFiltered {
...
func TestProcessIfEnabled(t *testing.T) {
	...
	result := k.ProcessIfEnabled(FeatureKalmanDecay, 100.0, 0)
	if result.WasFiltered {
...
func BenchmarkComparison_Filtered(b *testing.B) {
	EnableKalmanFiltering()
	defer DisableKalmanFiltering()
	
	k := NewKalman(DefaultConfig())
	b.ResetTimer()
	
	for i := 0; i < b.N; i++ {
		k.ProcessIfEnabled(FeatureKalmanDecay, float64(i), 0)
	}
}
