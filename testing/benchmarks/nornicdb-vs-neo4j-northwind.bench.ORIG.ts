...
async function loadNorthwindDataset(session: Session): Promise<void> {
  ...
  // Create Products with SUPPLIES relationships
  await session.run(`
    MATCH (s1:Supplier {supplierID: 1}), (c1:Category {categoryID: 1})
    CREATE (p1:Product {productID: 1, productName: 'Chai', unitPrice: 18.0, unitsInStock: 39})
    CREATE (p1)-[:PART_OF]->(c1)
    CREATE (s1)-[:SUPPLIES]->(p1)
    
    MATCH (s1:Supplier {supplierID: 1}), (c1:Category {categoryID: 1})
    CREATE (p2:Product {productID: 2, productName: 'Chang', unitPrice: 19.0, unitsInStock: 17})
    CREATE (p2)-[:PART_OF]->(c1)
    CREATE (s1)-[:SUPPLIES]->(p2)
    
    MATCH (s2:Supplier {supplierID: 2}), (c2:Category {categoryID: 2})
    CREATE (p3:Product {productID: 3, productName: 'Aniseed Syrup', unitPrice: 10.0, unitsInStock: 13})
    CREATE (p3)-[:PART_OF]->(c2)
    CREATE (s2)-[:SUPPLIES]->(p3)
    
    MATCH (s2:Supplier {supplierID: 2}), (c2:Category {categoryID: 2})
    CREATE (p4:Product {productID: 4, productName: 'Chef Anton Cajun Seasoning', unitPrice: 22.0, unitsInStock: 53})
    CREATE (p4)-[:PART_OF]->(c2)
    CREATE (s2)-[:SUPPLIES]->(p4)
    
    MATCH (s3:Supplier {supplierID: 3}), (c2:Category {categoryID: 2})
    CREATE (p5:Product {productID: 5, productName: 'Chef Anton Gumbo Mix', unitPrice: 21.35, unitsInStock: 0})
    CREATE (p5)-[:PART_OF]->(c2)
    CREATE (s3)-[:SUPPLIES]->(p5)
    
    MATCH (s3:Supplier {supplierID: 3}), (c2:Category {categoryID: 2})
    CREATE (p6:Product {productID: 6, productName: "Grandma's Boysenberry Spread", unitPrice: 25.0, unitsInStock: 120})
    CREATE (p6)-[:PART_OF]->(c2)
    CREATE (s3)-[:SUPPLIES]->(p6)
    
    MATCH (s4:Supplier {supplierID: 4}), (c7:Category {categoryID: 7})
    CREATE (p7:Product {productID: 7, productName: 'Uncle Bob Organic Dried Pears', unitPrice: 30.0, unitsInStock: 15})
    CREATE (p7)-[:PART_OF]->(c7)
    CREATE (s4)-[:SUPPLIES]->(p7)
    
    MATCH (s4:Supplier {supplierID: 4}), (c2:Category {categoryID: 2})
    CREATE (p8:Product {productID: 8, productName: 'Northwoods Cranberry Sauce', unitPrice: 40.0, unitsInStock: 6})
    CREATE (p8)-[:PART_OF]->(c2)
    CREATE (s4)-[:SUPPLIES]->(p8)
    
    MATCH (s5:Supplier {supplierID: 5}), (c4:Category {categoryID: 4})
    CREATE (p9:Product {productID: 9, productName: 'Queso Cabrales', unitPrice: 21.0, unitsInStock: 22})
    CREATE (p9)-[:PART_OF]->(c4)
    CREATE (s5)-[:SUPPLIES]->(p9)
    
    MATCH (s5:Supplier {supplierID: 5}), (c4:Category {categoryID: 4})
    CREATE (p10:Product {productID: 10, productName: 'Queso Manchego La Pastora', unitPrice: 38.0, unitsInStock: 86})
    CREATE (p10)-[:PART_OF]->(c4)
    CREATE (s5)-[:SUPPLIES]->(p10)
    
    MATCH (s6:Supplier {supplierID: 6}), (c8:Category {categoryID: 8})
    CREATE (p11:Product {productID: 11, productName: 'Konbu', unitPrice: 6.0, unitsInStock: 24})
    CREATE (p11)-[:PART_OF]->(c8)
    CREATE (s6)-[:SUPPLIES]->(p11)
    
    MATCH (s6:Supplier {supplierID: 6}), (c8:Category {categoryID: 8})
    CREATE (p12:Product {productID: 12, productName: 'Tofu', unitPrice: 23.25, unitsInStock: 35})
    CREATE (p12)-[:PART_OF]->(c8)
    CREATE (s6)-[:SUPPLIES]->(p12)
    
    MATCH (s7:Supplier {supplierID: 7}), (c3:Category {categoryID: 3})
    CREATE (p13:Product {productID: 13, productName: 'Pavlova', unitPrice: 17.45, unitsInStock: 29})
    CREATE (p13)-[:PART_OF]->(c3)
    CREATE (s7)-[:SUPPLIES]->(p13)
    
    MATCH (s7:Supplier {supplierID: 7}), (c6:Category {categoryID: 6})
    CREATE (p14:Product {productID: 14, productName: 'Alice Mutton', unitPrice: 39.0, unitsInStock: 0})
    CREATE (p14)-[:PART_OF]->(c6)
    CREATE (s7)-[:SUPPLIES]->(p14)
    
    MATCH (s8:Supplier {supplierID: 8}), (c3:Category {categoryID: 3})
    CREATE (p15:Product {productID: 15, productName: 'Chocolate Biscuits', unitPrice: 9.20, unitsInStock: 38})
    CREATE (p15)-[:PART_OF]->(c3)
    CREATE (s8)-[:SUPPLIES]->(p15)
    
    MATCH (s8:Supplier {supplierID: 8}), (c3:Category {categoryID: 3})
    CREATE (p16:Product {productID: 16, productName: 'Scones', unitPrice: 7.75, unitsInStock: 61})
    CREATE (p16)-[:PART_OF]->(c3)
    CREATE (s8)-[:SUPPLIES]->(p16)
  `);
  ...
  // Create Orders with PURCHASED relationships and ORDERS relationships to products
  await session.run(`
    MATCH (cu1:Customer {customerID: 'ALFKI'})
    CREATE (o1:Order {orderID: 10643, orderDate: '1997-08-25', shipCountry: 'Germany', shipCity: 'Berlin'})
    CREATE (cu1)-[:PURCHASED]->(o1)
    
    MATCH (cu2:Customer {customerID: 'ANATR'})
    CREATE (o2:Order {orderID: 10308, orderDate: '1996-09-18', shipCountry: 'Mexico', shipCity: 'México D.F.'})
    CREATE (cu2)-[:PURCHASED]->(o2)
    
    MATCH (cu3:Customer {customerID: 'ANTON'})
    CREATE (o3:Order {orderID: 10365, orderDate: '1996-11-27', shipCountry: 'Mexico', shipCity: 'México D.F.'})
    CREATE (cu3)-[:PURCHASED]->(o3)
    
    MATCH (cu4:Customer {customerID: 'AROUT'})
    CREATE (o4:Order {orderID: 10355, orderDate: '1996-11-15', shipCountry: 'UK', shipCity: 'London'})
    CREATE (cu4)-[:PURCHASED]->(o4)
    
    MATCH (cu5:Customer {customerID: 'BERGS'})
    CREATE (o5:Order {orderID: 10278, orderDate: '1996-08-12', shipCountry: 'Sweden', shipCity: 'Luleå'})
    CREATE (cu5)-[:PURCHASED]->(o5)
    
    MATCH (cu6:Customer {customerID: 'BLAUS'})
    CREATE (o6:Order {orderID: 10501, orderDate: '1997-04-09', shipCountry: 'Germany', shipCity: 'Mannheim'})
    CREATE (cu6)-[:PURCHASED]->(o6)
    
    MATCH (cu7:Customer {customerID: 'BLONP'})
    CREATE (o7:Order {orderID: 10265, orderDate: '1996-07-25', shipCountry: 'France', shipCity: 'Strasbourg'})
    CREATE (cu7)-[:PURCHASED]->(o7)
    
    MATCH (cu8:Customer {customerID: 'BOLID'})
    CREATE (o8:Order {orderID: 10326, orderDate: '1996-10-10', shipCountry: 'Spain', shipCity: 'Madrid'})
    CREATE (cu8)-[:PURCHASED]->(o8)
  `);
  
  // Create Order -> Product relationships (ORDERS with quantity)
  await session.run(`
    MATCH (o1:Order {orderID: 10643}), (p1:Product {productID: 1})
    CREATE (o1)-[:ORDERS {quantity: 15}]->(p1)
    
    MATCH (o1:Order {orderID: 10643}), (p9:Product {productID: 9})
    CREATE (o1)-[:ORDERS {quantity: 18}]->(p9)
    
    MATCH (o2:Order {orderID: 10308}), (p2:Product {productID: 2})
    CREATE (o2)-[:ORDERS {quantity: 10}]->(p2)
    
    MATCH (o2:Order {orderID: 10308}), (p11:Product {productID: 11})
    CREATE (o2)-[:ORDERS {quantity: 21}]->(p11)
    
    MATCH (o3:Order {orderID: 10365}), (p3:Product {productID: 3})
    CREATE (o3)-[:ORDERS {quantity: 60}]->(p3)
    
    MATCH (o3:Order {orderID: 10365}), (p4:Product {productID: 4})
    CREATE (o3)-[:ORDERS {quantity: 20}]->(p4)
    
    MATCH (o4:Order {orderID: 10355}), (p5:Product {productID: 5})
    CREATE (o4)-[:ORDERS {quantity: 25}]->(p5)
    
    MATCH (o4:Order {orderID: 10355}), (p6:Product {productID: 6})
    CREATE (o4)-[:ORDERS {quantity: 30}]->(p6)
    
    MATCH (o5:Order {orderID: 10278}), (p7:Product {productID: 7})
    CREATE (o5)-[:ORDERS {quantity: 16}]->(p7)
    
    MATCH (o5:Order {orderID: 10278}), (p8:Product {productID: 8})
    CREATE (o5)-[:ORDERS {quantity: 15}]->(p8)
    
    MATCH (o6:Order {orderID: 10501}), (p10:Product {productID: 10})
    CREATE (o6)-[:ORDERS {quantity: 20}]->(p10)
    
    MATCH (o6:Order {orderID: 10501}), (p12:Product {productID: 12})
    CREATE (o6)-[:ORDERS {quantity: 10}]->(p12)
    
    MATCH (o7:Order {orderID: 10265}), (p13:Product {productID: 13})
    CREATE (o7)-[:ORDERS {quantity: 40}]->(p13)
    
    MATCH (o7:Order {orderID: 10265}), (p14:Product {productID: 14})
    CREATE (o7)-[:ORDERS {quantity: 35}]->(p14)
    
    MATCH (o8:Order {orderID: 10326}), (p15:Product {productID: 15})
    CREATE (o8)-[:ORDERS {quantity: 8}]->(p15)
    
    MATCH (o8:Order {orderID: 10326}), (p16:Product {productID: 16})
    CREATE (o8)-[:ORDERS {quantity: 70}]->(p16)
  `);
}
